...
TEST_F(ProjectViewStateTests, CollapseAndExpandTrackHeights)
{
    const trackedit::TrackIdList tracks = getAllTracks();
    ASSERT_EQ(tracks.size(), 2) << "Expecting 2 tracks in test.aup4";
    ...
    EXPECT_EQ(m_projectViewState->trackHeight(tracks[0]).val, 50);
    EXPECT_EQ(m_projectViewState->trackHeight(tracks[1]).val, 60);
    ...
    EXPECT_EQ(m_projectViewState->trackHeight(tracks[0]).val, 88);
