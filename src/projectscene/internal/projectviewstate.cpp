...
void ProjectViewState::collapseAllTrackHeights()
{
    ...
    for (const trackedit::TrackId& trackId : prj->trackIdList()) {
        setTrackHeightInternal(trackId, trackHeight(trackId).val / 2, false);
    }
...
void ProjectViewState::expandAllTrackHeights()
{
    ...
    for (const trackedit::TrackId& trackId : prj->trackIdList()) {
        const int oldHeight = trackHeight(trackId).val;
        const int doubledHeight = oldHeight > std::numeric_limits<int>::max() / 2
                                  ? std::numeric_limits<int>::max()
                                  : oldHeight * 2;
        setTrackHeightInternal(trackId, std::min(doubledHeight, maxTrackHeight), false);
    }
...
{
    disengageAutoFitTrackHeights();
    setTrackHeightInternal(trackId, trackHeight(trackId).val / 2, true);
}
...
void ProjectViewState::expandTrackHeight(const trackedit::TrackId& trackId)
{
    ...
    setTrackHeightInternal(trackId, std::min(doubledHeight, maxTrackHeight), true);
}
